...
import { ElButton, ElButtonGroup, ElCol, ElContainer, ElDialog, ElIcon, ElInput, ElRow, ElTable, ElTableColumn, ElText, ElTooltip } from 'element-plus';
...
<template>
    <el-dialog title="选择路径" width="800">
        <el-container direction="vertical">
            ...
            <el-row>
                <el-col :span="24">
                    <el-table :data="entries" highlight-current-row @current-change="choose_entry"
                        @row-dblclick="enter_dir" :max-height="400">
                            ...
                            <template #default="scope">
                                <div style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
                                    <el-tooltip placement="top" :content="scope.row.name">
                                        <el-text style="user-select: none;">{{ scope.row.name }}</el-text>
                                    </el-tooltip>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column prop="updated_at" label="修改时间" width="180" sortable>
                            <template #default="scope">
                                <el-text style="user-select: none;">{{ dayjs(scope.row.updated_at).format('YYYY-MM-DD HH:mm:ss') }}</el-text>
                            </template>
                        </el-table-column>
                    </el-table>
                </el-col>
            ...
            <el-row>
                <el-col :span="20" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
